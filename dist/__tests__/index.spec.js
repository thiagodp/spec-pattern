...
describe('greater than', function () {
    it('right', function () {
        expect((new index_1.GreaterThan(0))
...
describe('starts with', function () {
    it('right - case sensitive', function () {
        expect((new index_1.StartsWith('Hello'))
            .isSatisfiedBy('Hello world')).toBeTruthy();
    });
    it('right - case insensitive', function () {
        expect((new index_1.StartsWith('Hello', true))
            .isSatisfiedBy('hello world')).toBeTruthy();
    });
    it('wrong - not in the start', function () {
        expect((new index_1.StartsWith('world'))
...
describe('ends with', function () {
    it('right - case sensitive', function () {
        expect((new index_1.EndsWith('world'))
            .isSatisfiedBy('Hello world')).toBeTruthy();
    });
    it('right - case insensitive', function () {
        expect((new index_1.EndsWith('World', true))
            .isSatisfiedBy('hello world')).toBeTruthy();
    });
    it('wrong - not in the end', function () {
        expect((new index_1.EndsWith('hello'))
            .isSatisfiedBy('Hello world')).toBeFalsy();
    });
    it('wrong - different case when not insensitive', function () {
        expect((new index_1.EndsWith('World'))
...
describe('contains', function () {
    it('right - case sensitive', function () {
        expect((new index_1.Contains('w'))
            .isSatisfiedBy('Hello world')).toBeTruthy();
    });
    it('right - case insensitive', function () {
        expect((new index_1.Contains('W', true))
            .isSatisfiedBy('hello world')).toBeTruthy();
    });
    it('wrong - not exists', function () {
        expect((new index_1.Contains('x'))
            .isSatisfiedBy('Hello world')).toBeFalsy();
    });
    it('wrong - different case when not insensitive', function () {
        expect((new index_1.Contains('W'))
...
describe('between', function () {
    describe('right', function () {
        it('min', function () {
            expect((new index_1.Between(-1, 1)).isSatisfiedBy(-1)).toBeTruthy();
        });
        it('max', function () {
            expect((new index_1.Between(-1, 1)).isSatisfiedBy(1)).toBeTruthy();
        });
        it('any value between', function () {
            expect((new index_1.Between(-1, 1)).isSatisfiedBy(0)).toBeTruthy();
        });
    });
    describe('wrong', function () {
        it('less than min', function () {
            expect((new index_1.Between(-1, 1)).isSatisfiedBy(-2)).toBeFalsy();
        });
    ...
    describe('composition', function () {
        describe('with not', function () {
            it('right - invert the logic', function () {
                expect((new index_1.Between(-1, 1))
                    .not()
                    .isSatisfiedBy(0)).toBeFalsy();
            });
        });
        describe('with or', function () {
            ...
            it('wrong - when in none', function () {
                expect((new index_1.Between(-3, -1))
                    .or(new index_1.Between(1, 3))
                    .isSatisfiedBy(0)).toBeFalsy();
            });
        });
        describe('with and', function () {
            it('right - when in both', function () {
                expect((new index_1.Between(-3, 0))
                    .and(new index_1.Between(0, 3))
                    .isSatisfiedBy(0)).toBeTruthy();
            });
        ...
        describe('with and not', function () {
            it('right - left side and not right', function () {
                expect((new index_1.Between(-1, 1))
                    .andNot(new index_1.Between(5, 7))
                    .isSatisfiedBy(0)).toBeTruthy();
            });
...
describe('length between', function () {
    it('right - min', function () {
        expect((new index_1.LengthBetween(2, 5))
...
describe('matches', function () {
    it('right', function () {
        expect((new index_1.Matches(/^foo$/))
            .isSatisfiedBy('foo')).toBeTruthy();
    });
    it('wrong', function () {
        expect((new index_1.Matches(/^foo$/))
            .isSatisfiedBy('bar')).toBeFalsy();
    });
});
